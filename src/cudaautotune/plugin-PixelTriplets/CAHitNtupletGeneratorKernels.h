...
public:
  ...
  void launchKernels(HitsOnCPU const& hh, TkSoA* tuples_d, std::unordered_map<std::string, int> launchConfigs, cudaStream_t cudaStream);

  void classifyTuples(HitsOnCPU const& hh, TkSoA* tuples_d, cudaStream_t cudaStream);
  void classifyTuples(HitsOnCPU const& hh, TkSoA* tuples_d, std::unordered_map<std::string, int> launchConfigs, cudaStream_t cudaStream);

  void fillHitDetIndices(HitsView const* hv, TkSoA* tuples_d, cudaStream_t cudaStream);
  void fillHitDetIndices(HitsView const* hv, TkSoA* tuples_d, std::unordered_map<std::string, int> launchConfigs, cudaStream_t cudaStream);

  void buildDoublets(HitsOnCPU const& hh, cudaStream_t stream);
  void buildDoublets(HitsOnCPU const& hh, std::unordered_map<std::string, int> launchConfigs, cudaStream_t stream);
