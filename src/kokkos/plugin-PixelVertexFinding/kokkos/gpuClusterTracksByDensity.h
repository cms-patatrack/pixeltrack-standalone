...
namespace KOKKOS_NAMESPACE {
  namespace gpuVertexFinder {
    ...
    KOKKOS_INLINE_FUNCTION void clusterTracksByDensityA(
    ...
    template <typename Histo>
    KOKKOS_INLINE_FUNCTION void clusterTracksByDensityB(
        Kokkos::View<ZVertices, KokkosExecSpace> vdata,
        Kokkos::View<WorkSpace, KokkosExecSpace> vws,
        Kokkos::View<Histo*, KokkosExecSpace> vhist,
        int minT,       // min number of neighbours to be "seed"
    ...
    void clusterTracksByDensityHost(Kokkos::View<ZVertices, KokkosExecSpace> vdata,
                                    Kokkos::View<WorkSpace, KokkosExecSpace> vws,
                                    int minT,       // min number of neighbours to be "seed"
                                    float eps,      // max absolute distance to cluster
                                    float errmax,   // max error to be "seed"
      ...
      Kokkos::parallel_for(
          policy, KOKKOS_LAMBDA(const member_type& team_member) {
            clusterTracksByDensityA(vdata, vws, vhist, minT, eps, errmax, chi2max, team_member);
          });
      ...
      Kokkos::parallel_for(
          policy, KOKKOS_LAMBDA(const member_type& team_member) {
            clusterTracksByDensityB(vdata, vws, vhist, minT, eps, errmax, chi2max, team_member);
          });
    }
  }  // namespace gpuVertexFinder
}  // namespace KOKKOS_NAMESPACE
