...
namespace KOKKOS_NAMESPACE {
  namespace BrokenLine {
    ...
    using namespace KOKKOS_NAMESPACE::Rfit;
  ...
  */
    KOKKOS_INLINE_FUNCTION Matrix2d RotationMatrix(double slope) {
      Matrix2d Rot;
  ...
  */
    KOKKOS_INLINE_FUNCTION void TranslateKarimaki(karimaki_circle_fit& circle, double x0, double y0, Matrix3d& Jacob) {
  ...
  */
    template <int N>
    KOKKOS_INLINE_FUNCTION MatrixNd<N> MatrixC_u(const VectorNd<N>& w,
                                                 const VectorNd<N>& S,
  ...
  */
    ...
    KOKKOS_INLINE_FUNCTION void BL_Line_fit(const M6xN& hits_ge,
      ...
      for (i = 0; i < n; i++) {
        ...
        if (i > 0 && i < n - 1)
          line_results.chi2 += sqr(u(i - 1) / (S(i) - S(i - 1)) -
                                   u(i) * (S(i + 1) - S(i - 1)) / ((S(i + 1) - S(i)) * (S(i) - S(i - 1))) +
                                   u(i + 1) / (S(i + 1) - S(i))) /
                               VarBeta(i);
      }
  ...
  */
    template <int N>
    inline helix_fit BL_Helix_fit(const Matrix3xNd<N>& hits,
