    ...
    namespace impl {
      ...
      void ExecSpaceSpecific<Kokkos::Cuda>::enqueueCallback(edm::WaitingTaskWithArenaHolder holder) {
        cudaCheck(cudaStreamAddCallback(
            space_->space().cuda_stream(), cudaScopedContextCallback, new CallbackData{std::move(holder), device()}, 0));
      }
      ...
      void ExecSpaceSpecific<Kokkos::Cuda>::synchronizeWith(ExecSpaceSpecific const& other) {
        ...
        if (other.space_->space().cuda_stream() != space_->space().cuda_stream()) {
          ...
          if (not other.isAvailable()) {
            ...
            cudaCheck(cudaStreamWaitEvent(space_->space().cuda_stream(), other.event_.get(), 0),
