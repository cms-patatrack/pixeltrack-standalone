...
namespace gpuClustering {
  ...
  void findClus(Kokkos::View<uint16_t*, ExecSpace> id,           // module id of each pixel
    ...
    Kokkos::parallel_for("init_hist_off",team_policy(execSpace,league_size,team_size),
      KOKKOS_LAMBDA(const member_type& teamMember){
        Kokkos::parallel_for(Kokkos::TeamThreadRange(teamMember,loop_count),
          [&](const int index){
            d_hist(teamMember.league_rank()).off[index] = 0;
          });
      });
    ...
    Kokkos::parallel_for("findClus_msize",
      team_policy(execSpace,league_size,team_size).set_scratch_size(shared_view_level,Kokkos::PerTeam(shared_view_bytes)),
      KOKKOS_LAMBDA(const member_type& teamMember){
        ...
        while (more) {
          if (1 == nloops % 2) {
            for (uint16_t j = teamMember.team_rank(), k = 0U; j < d_hist(teamMember.league_rank()).size(); j += teamMember.team_size(), ++k) {
              auto p = d_hist(teamMember.league_rank()).begin() + j;
              auto i = *p + firstPixel;
              auto m = clusterId(i);
              while (m != clusterId(m)){
