  ...
  public:
    ...
    EDPutTokenT<T> produces() {
      ...
      if (not succeeded.second) {
        throw std::runtime_error(std::string("ProductRegistry::produces(): Product of type ") + typeid(T).name() + " already exists");
      }
    ...
    EDGetTokenT<T> consumes() {
      ...
      if (found == typeToIndex_.end()) {
        throw std::runtime_error(std::string("ProductRegistry::consumes(): Product of type ") + typeid(T).name() + " is not produced");
      }
