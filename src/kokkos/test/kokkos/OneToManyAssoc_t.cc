...
void countMulti(Kokkos::View<T,ExecSpace> assoc,const uint32_t& n,ExecSpace const& execSpace) {
  Kokkos::parallel_for("countMulti",Kokkos::RangePolicy<ExecSpace>(execSpace,0,n),
...
void verifyMulti(Kokkos::View<Multiplicity,ExecSpace> m1,
                 Kokkos::View<Multiplicity,ExecSpace> m2,ExecSpace const& execSpace) {
  Kokkos::parallel_for("verifyMulti",Kokkos::RangePolicy<ExecSpace>(execSpace,0,Multiplicity::totbins()),
    KOKKOS_LAMBDA(const int& i){
...
void count(Kokkos::View<uint16_t**,ExecSpace> const tk,
           ...
           const uint32_t& n,ExecSpace const& execSpace) {

  Kokkos::parallel_for("count",Kokkos::RangePolicy<ExecSpace>(execSpace,0,4*n),
    KOKKOS_LAMBDA(const int& i){
      uint32_t k = i / 4;
      auto j = i - 4 * k;
      assert(j < 4);
...
void fill(Kokkos::View<uint16_t**,ExecSpace> const tk,
          Kokkos::View<Assoc,ExecSpace> assoc,
          const uint32_t& n,ExecSpace const& execSpace) {
  Kokkos::parallel_for("fill",Kokkos::RangePolicy<ExecSpace>(execSpace,0,4*n),
    KOKKOS_LAMBDA(const int& i){
      uint32_t k = i / 4;
      auto j = i - 4 * k;
      assert(j < 4);
...
void verify(Kokkos::View<Assoc,ArrayLayout,ExecSpace> assoc) { assert(assoc().size() < Assoc::capacity()); }
...
void fillBulk(Kokkos::View<AtomicPairCounter,ExecSpace> apc,
              ...
              Kokkos::View<Assoc,ExecSpace> assoc, 
              const uint32_t& n,ExecSpace const& execSpace) {
  Kokkos::parallel_for("fillBulk",Kokkos::RangePolicy<ExecSpace>(execSpace,0,n),
    KOKKOS_LAMBDA(const int& i){
...
int main() {
  ...
  Kokkos::parallel_for("init",Kokkos::RangePolicy<KokkosExecSpace>(KokkosExecSpace(),0,1),
    KOKKOS_LAMBDA(const int& i){
      ave_d() = 0;
      imax_d() = 0;
      n_d() = 0;
  ...
  std::cout << "filled with " << n_h() << " elements " << double(ave_h()) / n_h() << ' ' << imax_h() << ' ' << nz_h() << std::endl;
