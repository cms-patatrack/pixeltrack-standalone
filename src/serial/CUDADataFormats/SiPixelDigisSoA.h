...
public:
  ...
  public:
    // DeviceConstView() = default;
     ...
     inline  uint16_t xx(int i) const { return __ldg(xx_ + i); }
     inline  uint16_t yy(int i) const { return __ldg(yy_ + i); }
     inline  uint16_t adc(int i) const { return __ldg(adc_ + i); }
     inline  uint16_t moduleInd(int i) const { return __ldg(moduleInd_ + i); }
     inline  int32_t clus(int i) const { return __ldg(clus_ + i); }
