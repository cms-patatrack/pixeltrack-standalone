...
namespace cms {
  namespace cuda {
    ...
    private:
      ...
      explicit Product(int device, std::shared_ptr<impl::StreamSharingHelper> stream, SharedEventPtr event, T data)
          : ProductBase(device, std::move(stream), std::move(event)), data_(std::move(data)) {}

      template <typename... Args>
      explicit Product(int device,
                       std::shared_ptr<impl::StreamSharingHelper> stream,
