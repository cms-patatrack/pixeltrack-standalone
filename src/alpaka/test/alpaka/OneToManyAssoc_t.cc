...
struct countMultiLocal {
  ...
  ALPAKA_FN_ACC void operator()(const T_Acc &acc, TK const* __restrict__ tk, Multiplicity* __restrict__ assoc, uint32_t n) const {
    ...
    const auto& [firstElementIdxNoStride, endElementIdxNoStride] = cms::alpakatools::element_global_index_range(acc, Vec1::all(n));
    uint32_t endElementIdx = endElementIdxNoStride[0u];
    for (uint32_t firstElementIdx = firstElementIdxNoStride[0u]; firstElementIdx < n; firstElementIdx += gridDimension) {
      for (uint32_t i = firstElementIdx; i < endElementIdx; ++i) {
...
struct verifyMulti {
  ...
  ALPAKA_FN_ACC void operator()(const T_Acc &acc, Multiplicity* __restrict__ m1, Multiplicity* __restrict__ m2) const {
    ...
    const uint32_t gridDimension(alpaka::workdiv::getWorkDiv<alpaka::Grid, alpaka::Elems>(acc)[0u]);
    const auto& [firstElementIdxNoStride, endElementIdxNoStride] = cms::alpakatools::element_global_index_range(acc, Vec1::all(maxNumberOfElements));
    uint32_t endElementIdx = endElementIdxNoStride[0u];
    for (uint32_t firstElementIdx = firstElementIdxNoStride[0u]; firstElementIdx < maxNumberOfElements; firstElementIdx += gridDimension) {
      for (uint32_t i = firstElementIdx; i < endElementIdx; ++i) {
...
struct count {
  template <typename T_Acc>
  ALPAKA_FN_ACC void operator()(const T_Acc &acc, TK const* __restrict__ tk, Assoc* __restrict__ assoc, uint32_t n) const {
...
struct fillBulk {
  ...
  ALPAKA_FN_ACC void operator()(const T_Acc &acc, cms::alpakatools::AtomicPairCounter* apc, TK const* __restrict__ tk, Assoc* __restrict__ assoc, uint32_t n) const {
    const uint32_t gridDimension(alpaka::workdiv::getWorkDiv<alpaka::Grid, alpaka::Elems>(acc)[0u]);
    const auto& [firstElementIdxNoStride, endElementIdxNoStride] = cms::alpakatools::element_global_index_range(acc, Vec1::all(n));
    uint32_t endElementIdx = endElementIdxNoStride[0u];
    for (uint32_t firstElementIdx = firstElementIdxNoStride[0u]; firstElementIdx < n; firstElementIdx += gridDimension) {
      for (uint32_t k = firstElementIdx; k < endElementIdx; ++k) {
...
struct verifyBulk {
  template <typename T_Acc, typename Assoc>
  ALPAKA_FN_ACC void operator()(const T_Acc &acc, Assoc const* __restrict__ assoc, cms::alpakatools::AtomicPairCounter const* apc) const {
...
int main() {
  ...
  alpaka::queue::enqueue(queue,
    alpaka::kernel::createTaskKernel<Acc1>(workDivTotBins,
    verifyMulti(),
    alpaka::mem::view::getPtrNative(m1_dbuf),
    alpaka::mem::view::getPtrNative(m2_dbuf)
    ));
