...
namespace ALPAKA_ACCELERATOR_NAMESPACE{
    ...
    public:
    SiPixelGainCalibrationForHLTGPU(SiPixelGainForHLTonGPU gain, std::vector<SiPixelGainForHLTonGPU_DecodingStructure> gainData):
    ...
    ~SiPixelGainCalibrationForHLTGPU() = default;
        ...
        const SiPixelGainForHLTonGPU *getGPUProductAsync(Queue *queue) const {
          const auto &data = gpuData_.dataForDeviceAsync(queue, [this](Queue *queue) {
            ...
            std::memcpy(alpaka::getPtrNative(gpuData.gainDataOnGPU), alpaka::getPtrNative(gainForHLTonHost), sizeof(SiPixelGainForHLTonGPU));

            alpaka::getPtrNative(gpuData.gainDataOnGPU)->v_pedestals =  alpaka::getPtrNative(gpuData.v_pedestalsGPU);
            alpaka::prepareForAsyncCopy(gpuData.gainDataOnGPU);
    ...
    struct GPUData {    
        ...
        GPUData(Queue &queue, unsigned int numDecodingStructures)
            : gainForHLTonGPU{::cms::alpakatools::allocDeviceBuf<SiPixelGainForHLTonGPU>(alpaka::getDev(queue), 1u)},
              ...
              v_pedestalsGPU{::cms::alpakatools::allocDeviceBuf<SiPixelGainForHLTonGPU_DecodingStructure>(alpaka::getDev(queue),numDecodingStructures)}{};
